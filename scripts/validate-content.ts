...
for (const item of content.media) { if (ids.has(item.id)) errors.push(`Duplicate media ID: ${item.id}`); ids.add(item.id); const source = resolve(content.mode === "demo" ? "public" : (process.env.PRIVATE_MEDIA_ROOT ?? "private-media"), item.privatePath); if (!existsSync(source)) errors.push(`Missing media for ${item.id}; expected ${source}`); }
...
for (const voice of content.voices) if (!ids.has(voice.mediaId)) errors.push(`Voice ${voice.id} references unknown media: ${voice.mediaId}`);
