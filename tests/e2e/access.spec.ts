...
test("loads every private photo through the protected route", async ({ page }) => { await enterStory(page); const imageIds = ["photo-first","photo-sky","photo-cafe","photo-hands","photo-car","photo-rose","photo-escalator","photo-close","photo-night","photo-smile","photo-mirror","photo-city","photo-flower","photo-daylight","photo-garden","photo-green","photo-peace","photo-outdoor","photo-tree","photo-moon","photo-glasses","photo-filter","photo-last","note-universe","note-heart","note-only-you","note-rare","note-prettiest"]; const results = await page.evaluate(async (ids) => Promise.all(ids.map((id) => new Promise<boolean>((resolve) => { const image = new Image(); image.onload = () => resolve(image.naturalWidth > 0 && image.naturalHeight > 0); image.onerror = () => resolve(false); image.src = `/api/media/${id}`; }))), imageIds); expect(results.every(Boolean)).toBe(true); });
